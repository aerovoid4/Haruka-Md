...
async function startHaruka() {
    ...
    if (usePairingCode && !haruka.authState.creds.registered) {
        ...
        for (let users of celled) {
            if (users.isGroup == false) {
                if (users.status == "offer") {
                    const repf = await haruka.sendTextWithMentions(users.from, `*${haruka.user.name}* tidak bisa menerima panggilan ${users.isVideo ? `video` : `suara`}. Maaf @${users.from.split('@')[0]} kamu akan diblokir. Silahkan hubungi Owner membuka blok !`)
                    haruka.sendContact(users.from, repf)
                    //await sleep(8000)
                    //await haruka.updateBlockStatus(users.from, "block")
                }
            }
        }
    })
    ...
    haruka.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            for (let mek of chatUpdate.messages) {
                ...
                require("./case")(haruka, m, chatUpdate, mek, store, setting, isSetWelcome, getTextSetWelcome, set_welcome_db, set_left_db, isSetLeft, getTextSetLeft, _welcome, _left, antidelete, antionce)
            }
        } catch (err) {
            console.log(err)
        }
    ...
    haruka.ev.process(async (events) => {
    ...
    haruka.sendContact = async (jid, quoted) => {
        haruka.sendMessage(jid, { contacts: { displayName: "Owner", contacts: contacts } }, { quoted: quoted });
    }
